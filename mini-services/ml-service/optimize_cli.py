#!/usr/bin/env python3
"""
Standalone optimize script — reads JSON from stdin, writes JSON to stdout.
No HTTP server needed. Called as a subprocess from Next.js.

Usage:
  echo '{"transcript":[...],"plannedCourses":[...]}' | python optimize_cli.py

The Next.js API route spawns this as a subprocess.
"""
...
def predict_distribution(sks, difficulty, context, models):
    md = models["model2"]
    features = [[difficulty["difficulty_score"], difficulty["difficulty_cluster"], context["currentIpk"],
...
def optimize(body):
    ...
    cur_ipk = 0; total_sks = 0; ips_last = 0; ips_trend = 0; last_sem = 0
    ...
    context = {"currentIpk": round(cur_ipk, 3), "ipsLast": round(ips_last, 3),
               "ipsTrend": round(ips_trend, 3), "totalSks": total_sks, "semesterNumber": last_sem + 1}

    courses_data = []
    for c in planned:
        diff = get_difficulty(c.get("courseName", ""), models)
        dist = predict_distribution(c.get("sks", 3), diff, context, models)
        courses_data.append({
            "courseId": c.get("courseName", ""), "courseName": c.get("courseName", ""),
            "normalizedName": c.get("courseName", ""), "sks": c.get("sks", 3),
            "difficultyScore": diff["difficulty_score"], "difficultyLabel": diff["difficulty_label"],
            "distribution": dist,
        })

    scenarios = []
    best_delta = 0; best_ipk = cur_ipk; best_sc = "serius"
    for sn, eff in SCENARIO_EFFORT.items():
        ...
        if bd > best_delta:
            best_delta = bd; best_ipk = bi; best_sc = sn
        scenarios.append({
    ...
    classified = [{"courseName": c["courseName"], "major": "Umum",
                   ...
                   "difficultyLabel": c["difficultyLabel"], "difficultyScore": c["difficultyScore"],
